...
export const POST: RequestHandler = async ({ request, locals: { supabase, safeGetSession } }) => {
	...
	try {
		...
		// Získáme revoked nebo cleared token z databáze
		...
		if (!revokedTokens || revokedTokens.length === 0) {
			return json({
				success: true,
				message: 'Žádné revoked nebo cleared tokeny k testování',
				hasRevokedTokens: false
			});
		}

		const tokenData = revokedTokens[0];
		console.log('🔍 Testing revoked/cleared token for:', tokenData.account_email);
